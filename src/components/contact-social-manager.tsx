...
const socialPlatforms: Array<{ value: string; label: string; icon: string; names: LocalizedText }> = [
  ...
  { value: 'x', label: 'X', icon: 'x', names: { fr: 'X', en: 'X', ar: 'X' } },
  { value: 'linkedin', label: 'LinkedIn', icon: 'linkedin', names: { fr: 'LinkedIn', en: 'LinkedIn', ar: 'LinkedIn' } },
  { value: 'tripadvisor', label: 'Tripadvisor', icon: 'tripadvisor', names: { fr: 'Tripadvisor', en: 'Tripadvisor', ar: 'Tripadvisor' } },
...
function socialDraft(social?: AdminUiSocialDto): SocialDraft {
  return social ? { ...social, iconMediaId: social.iconMediaId ?? null, label: { ...social.label } } : { id: '', platform: '', label: { ...emptyLabel }, url: '', handle: '', icon: '', iconMediaId: null, enabled: true, sortOrder: 0 };
}
...
export function ContactSocialManager({ serverActions }: { serverActions: ServerActions }) {
  ...
  const load = async () => {
    const [nextContacts, nextSocialLinks, nextMedia] = await Promise.all([
    ...
    setMedia(nextMedia.filter((asset) => asset.type === 'image'));
  };
...
function SocialSection({ socialLinks, media, draft, editingId, busy, onMediaAdded, onCreate, onEdit, onCancel, onChange, onSave, onMove, onDelete, onToggle }: { socialLinks: AdminUiSocialDto[]; media: AdminUiMediaDto[]; draft: SocialDraft | null; editingId: string | null; busy: boolean; onMediaAdded?: (asset: AdminUiMediaDto) => void; onCreate: () => void; onEdit: (social: AdminUiSocialDto) => void; onCancel: () => void; onChange: (draft: SocialDraft | null) => void; onSave: () => void; onMove: (index: number, direction: -1 | 1) => void; onDelete: (id: string) => void; onToggle: (social: AdminUiSocialDto) => void }) {
  return <section className="admin-contact-social-section"><div className="admin-list-toolbar"><div><p className="admin-eyebrow">Social links</p><p className="admin-list-count">{socialLinks.length} social links</p></div><button type="button" className="admin-primary-button" onClick={onCreate}>Add social link</button></div><div className="admin-item-list">{socialLinks.map((social, index) => <article className="admin-item-row" key={social.id}><div className="admin-item-order">{index + 1}</div><div className="admin-item-main"><div className="admin-item-title"><PlatformIcon kind="social" value={social.platform} className="admin-platform-icon" label={social.label.en || social.label.fr || social.label.ar || social.platform} /><h2>{social.label.en}</h2><span className={social.enabled ? 'admin-state is-active' : 'admin-state'}>{social.enabled ? 'Enabled' : 'Disabled'}</span></div><p className="admin-item-meta"><span>{social.platform}</span><span>{social.url}</span></p></div><div className="admin-item-actions"><button type="button" className="admin-icon-button" disabled={busy || index === 0} onClick={() => onMove(index, -1)} aria-label="Move social link up">Up</button><button type="button" className="admin-icon-button" disabled={busy || index === socialLinks.length - 1} onClick={() => onMove(index, 1)} aria-label="Move social link down">Down</button><button type="button" className="admin-text-button" onClick={() => onEdit(social)}>Edit</button><button type="button" className="admin-text-button" onClick={() => onToggle(social)}>{social.enabled ? 'Disable' : 'Enable'}</button><button type="button" className="admin-text-button is-danger" onClick={() => onDelete(social.id)}>Remove</button></div></article>)}</div>{draft ? <SocialEditor draft={draft} media={media} editingId={editingId} busy={busy} onMediaAdded={onMediaAdded} onChange={onChange} onCancel={onCancel} onSave={onSave} /> : null}</section>;
}

function SocialEditor({ draft, editingId, media, busy, onMediaAdded, onChange, onCancel, onSave }: { draft: SocialDraft; media: AdminUiMediaDto[]; editingId: string | null; busy: boolean; onMediaAdded?: (asset: AdminUiMediaDto) => void; onChange: (draft: SocialDraft) => void; onCancel: () => void; onSave: () => void }) {
  return <form className="admin-item-editor" onSubmit={(event) => { event.preventDefault(); onSave(); }}><div className="admin-editor-heading"><div><p className="admin-eyebrow">{editingId ? 'Edit social link' : 'New social link'}</p><h2>{editingId ? 'Update social link' : 'Create social link'}</h2></div><button type="button" className="admin-text-button" onClick={onCancel}>Cancel</button></div><div className="admin-item-editor-grid"><label className="admin-select-field">Platform<select required value={draft.platform} onChange={(event) => onChange({ ...draft, platform: event.target.value })}><option value="">Choose a platform</option>{socialPlatforms.map((platform) => <option key={platform.value} value={platform.value}>{platform.label}</option>)}</select></label><label className="admin-field">URL<input required type="url" value={draft.url} onChange={(event) => onChange({ ...draft, url: event.target.value })} /></label><label className="admin-checkbox-label"><input type="checkbox" checked={draft.enabled} onChange={(event) => onChange({ ...draft, enabled: event.target.checked })} /> Active</label></div><ContextualMediaField label="Custom icon (optional)" helpText="The platform icon is automatic. Upload only when you want a custom icon." automaticLabel="Automatic platform icon" value={draft.iconMediaId} media={media} allowedTypes={['image']} onMediaAdded={onMediaAdded} onChange={(iconMediaId) => onChange({ ...draft, iconMediaId })} />{draft.platform === 'custom' ? <label className="admin-field">Custom platform name<input required value={draft.label.en ?? ''} onChange={(event) => onChange({ ...draft, label: { fr: event.target.value, en: event.target.value, ar: event.target.value } })} /></label> : null}<div className="admin-editor-actions"><button type="submit" className="admin-primary-button" disabled={busy}>{busy ? 'Saving...' : 'Save changes'}</button></div></form>;
}
