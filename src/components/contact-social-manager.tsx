...
export function ContactSocialManager({ serverActions }: { serverActions: ServerActions }) {
  ...
  const saveContact = async () => {
    ...
    try {
      const { id: _id, ...input } = contactDraftState;
  ...
  const saveSocial = async () => {
    ...
    setState('saving'); setError('');
    try {
      const { id: _id, ...input } = socialDraftState;
...
function SocialEditor({ draft, editingId, busy, onChange, onCancel, onSave }: { draft: SocialDraft; editingId: string | null; busy: boolean; onChange: (draft: SocialDraft) => void; onCancel: () => void; onSave: () => void }) {
  return <form className="admin-item-editor" onSubmit={(event) => { event.preventDefault(); onSave(); }}><div className="admin-editor-heading"><div><p className="admin-eyebrow">{editingId ? 'Edit social link' : 'New social link'}</p><h2>{editingId ? 'Update social link' : 'Create social link'}</h2></div><button type="button" className="admin-text-button" onClick={onCancel}>Cancel</button></div><p className="admin-field-hint-text">ID generated automatically when saved.</p><div className="admin-item-editor-grid"><label className="admin-field">Platform<input required placeholder="Instagram, TikTok, custom..." value={draft.platform} onChange={(event) => onChange({ ...draft, platform: event.target.value })} /></label><label className="admin-field">URL<input required type="url" value={draft.url} onChange={(event) => onChange({ ...draft, url: event.target.value })} /></label><label className="admin-field">Handle<input value={draft.handle ?? ''} onChange={(event) => onChange({ ...draft, handle: event.target.value })} /></label><label className="admin-field">Icon/logo reference<input value={draft.icon ?? ''} onChange={(event) => onChange({ ...draft, icon: event.target.value })} /></label><label className="admin-checkbox-label"><input type="checkbox" checked={draft.enabled} onChange={(event) => onChange({ ...draft, enabled: event.target.checked })} /> Enabled</label></div><LocalizedFieldGroup id="social-label" label="Label" value={draft.label} errors={{}} onChange={(locale, value) => onChange({ ...draft, label: { ...draft.label, [locale]: value } })} /><div className="admin-editor-actions"><button type="submit" className="admin-primary-button" disabled={busy}>{busy ? 'Saving...' : 'Save social link'}</button></div></form>;
}
