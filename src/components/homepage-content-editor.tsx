...
export function HomepageContentEditor({ serverActions }: { serverActions: ServerActions }) {
  ...
  const isDirty = isHomepageContentDirty(draft, saved);
  ...
  return <HomepageContentForm draft={draft} saved={saved} state={state} error={error} onChange={update} onReset={reset} onSave={() => void save()} />;
}
