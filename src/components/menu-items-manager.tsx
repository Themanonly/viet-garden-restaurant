...
export function ItemEditor({ draft, editingId, categories, media, state, errors, onMediaAdded, onChange, onCancel, onSave }: { draft: ItemDraft; editingId: string | null; categories: AdminUiCategoryDto[]; media: AdminUiMediaDto[]; state: ManagerState; errors: Record<string, string[]>; onMediaAdded?: (asset: AdminUiMediaDto) => void; onChange: (draft: ItemDraft) => void; onCancel: () => void; onSave: () => void }) {
  ...
  return (
    <form className="admin-item-editor" ref={editorRef} onSubmit={(event) => { event.preventDefault(); onSave(); }}>
      ...
      <ContextualMediaField label="Item image (optional)" helpText="Upload a dish photo here, or choose one already stored." value={draft.mediaId} media={media} allowedTypes={['image']} error={errors.mediaId} onMediaAdded={onMediaAdded} onChange={(mediaId) => onChange({ ...draft, mediaId: mediaId || undefined })} />
