...
const channelNames: Record<OrderingChannelType, LocalizedText> = { glovo: { fr: 'Glovo', en: 'Glovo', ar: 'Glovo' }, yassir: { fr: 'Yassir', en: 'Yassir', ar: 'ياسر' }, 'uber-eats': { fr: 'Uber Eats', en: 'Uber Eats', ar: 'أوبر إيتس' }, direct: { fr: 'Commande en ligne', en: 'Order online', ar: 'الطلب عبر الإنترنت' }, whatsapp: { fr: 'WhatsApp', en: 'WhatsApp', ar: 'واتساب' }, other: { ...emptyLabel } };
...
export function OrderingChannelsManager({ serverActions }: { serverActions: ServerActions }) {
  ...
  const save = async () => {
    ...
    try {
      const { id: _id, ...input } = { ...draft, name: draft.name.en || draft.name.fr || draft.name.ar ? draft.name : channelNames[draft.type], ctaText: draft.ctaText?.en || draft.ctaText?.fr || draft.ctaText?.ar ? draft.ctaText : channelNames[draft.type] };
  ...
  return <section className="admin-contact-social-section" aria-labelledby="ordering-channels-title">
    ...
    {draft ? <OrderingChannelEditor draft={draft} editingId={editingId} media={media} busy={state === 'saving'} onMediaAdded={(asset) => setMedia((current) => [...current.filter((item) => item.id !== asset.id), asset])} onChange={setDraft} onCancel={() => { setDraft(null); setEditingId(null); }} onSave={save} /> : null}
...
function OrderingChannelEditor({ draft, editingId, media, busy, onMediaAdded, onChange, onCancel, onSave }: { draft: OrderingDraft; editingId: string | null; media: AdminUiMediaDto[]; busy: boolean; onMediaAdded?: (asset: AdminUiMediaDto) => void; onChange: (draft: OrderingDraft) => void; onCancel: () => void; onSave: () => void }) {
  return <form className="admin-item-editor" onSubmit={(event) => { event.preventDefault(); onSave(); }}><div className="admin-editor-heading"><div><p className="admin-eyebrow">{editingId ? 'Edit ordering channel' : 'New ordering channel'}</p><h2>{editingId ? 'Update ordering channel' : 'Create ordering channel'}</h2></div><button type="button" className="admin-text-button" onClick={onCancel}>Cancel</button></div><div className="admin-item-editor-grid"><label className="admin-select-field">Type<select value={draft.type} onChange={(event) => onChange({ ...draft, type: event.target.value as OrderingChannelType })}>{channelTypes.map((type) => <option key={type.value} value={type.value}>{type.label}</option>)}</select></label><label className="admin-field">URL or deep link<input required type="url" value={draft.url} onChange={(event) => onChange({ ...draft, url: event.target.value })} /></label><label className="admin-checkbox-label"><input type="checkbox" checked={draft.enabled} onChange={(event) => onChange({ ...draft, enabled: event.target.checked })} /> Active</label></div><ContextualMediaField label="Custom logo (optional)" helpText="The channel logo is automatic. Upload only when you want a custom logo." automaticLabel="Automatic channel logo" value={draft.logoMediaId} media={media} allowedTypes={['image']} onChange={(logoMediaId) => onChange({ ...draft, logoMediaId: logoMediaId || undefined })} />{draft.type === 'other' ? <label className="admin-field">Custom channel name<input required value={draft.name.en ?? ''} onChange={(event) => onChange({ ...draft, name: { fr: event.target.value, en: event.target.value, ar: event.target.value } })} /></label> : null}<div className="admin-editor-actions"><button type="submit" className="admin-primary-button" disabled={busy}>{busy ? 'Saving...' : 'Save changes'}</button></div></form>;
}
