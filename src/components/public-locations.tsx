...
export function PublicLocations({ locations, locale }: { locations: Location[]; locale: Locale }) {
  ...
  return <section id="locations" className="locations-section" aria-labelledby="locations-title"><div className="locations-heading"><p className="identity-eyebrow">{localizedText(copy.eyebrow, locale)}</p><h2 id="locations-title">{localizedText(title, locale)}</h2></div><div className="locations-grid">{orderedLocations.map((location) => {
